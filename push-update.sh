...
updates_dir=/data/apex_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have ApexOS-VERSION-DATE-TYPE-* as format"
...
# Assume ApexOS-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.apex.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.apex.updater/databases/updates.db" \
